  ...
  export const ChatReadCount = async (uid: string | null) => {
    ...
    if (noReadError) {
      ...
      noReadData.forEach(message => {
        ...
        const roomLastRead = last_read.find(entry => entry.room_id === roomId)?.last_read_at;

        if (roomLastRead && message.created_at > roomLastRead) {
            noReadMessageCount[roomId] = noReadMessageCount[roomId] ? noReadMessageCount[roomId] + 1 : 1;
        }
      });

      noReadMessageTotal = Object.values(noReadMessageCount).reduce((total, count) => total + count, 0);
