<script lang="ts">
    ...
    switch (check_id) {
      case "priority":
        if (checked) {
          ...
          query_priority = query_priority.filter(
  ...
  const handle_todo_search = async () => {
    ...
    if (query_priority.length > 0) {
      filter_data = filter_data.filter((todo) =>
        query_priority.includes(todo.priority)
      );
    }
    if (query_completed) {
